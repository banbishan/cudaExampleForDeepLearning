...
int main(){
    ...
    cov<<<blockNum, threadNum>>>(imgGpu, kernelGpu, resultGpu, width, height, kernelSize);

    HANDLE_ERROR(cudaMemcpy(result, resultGpu, kernelSize * kernelSize * sizeof(float),cudaMemcpyHostToDevice));
